...
# Repo conventions (spec 08 §Repo layout)
...
| `frontend/` | 05: Next.js app | `frontend/src/api/schema.ts` is generated |
...
| `backlog/` | Backlog.md store | CLI only (`decision-records`) |
| `.claude/` | Agents, skills, commands, hooks, learnings | Committed; linted by `lint_tooling.py` |
| `deploy/` | Dockerfiles, `compose.yml` | |
| `data/` | `cache/`, `snapshots/`, `indexes/`, `records.sqlite` | **Gitignored. Never committed.** Snapshots and indexes are immutable |
...
## Names
- **Branches:** `<type>/<slug>`, where type ∈ `feat`, `fix`, `chore`, `docs`, `test`, `refactor`, `perf`
...
## Gotchas
- Spec 08 §Git still reads `feature → PR → main`. `CLAUDE.md` and `enforce-pr-workflow.sh` add `dev` in
  between; follow them.
- Spec 08 lists `docs/decisions/`; new decisions go through `backlog decision create`. Verify at
  implementation time whether `docs/decisions/` is retired before adding files there.
